...
impl Tracker {
   pub async fn get_peers(tracker_addr: String, left: u32, infohash: [u8; 20]) ->  anyhow::Result<Vec<SocketAddrV4>> {
       ...
       match serde_bencode::from_bytes(&body) {
           Ok(TrackerResponse::Error(e)) => {
               Err(anyhow!("tracker responded with error: {}", e.failure_reason))
           }
